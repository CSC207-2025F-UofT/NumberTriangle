 ...
 *
 * The structure is shown below. Observe that the parents of e are b and c,
 * whereas
 * d and f each only have one parent. Each row is complete and will never be
 * missing
 ...
 * h i j k
 *
 * Also note that this data structure is minimally defined and is only intended
 * to
 * be constructed using the loadTriangle method, which you will implement
 * in this file. We have not included any code to enforce the structure noted
 * above,
...
public class NumberTriangle {
     ...
     *
     * You can decide if you want to use a recursive or an iterative approach in
     * your solution.
     *
     * You can assume that:
     ...
     * @return the topmost NumberTriangle object in the NumberTriangle structure
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        List<NumberTriangle> currLevel = new ArrayList<NumberTriangle>();
        ...
        top = new NumberTriangle(Integer.parseInt(lineParts[0]));
        currLevel.add(top);
        while (line != null) {
            // remove when done; this line is included so running starter code prints the
            ...
            List<NumberTriangle> newCurr = new ArrayList<>();
            for (String s : lineParts) {
                newCurr.add(new NumberTriangle(Integer.parseInt(s)));
            }
            for (int i = 0; i < lineParts.length - 1; i += 1) {
                currLevel.get(i).left = newCurr.get(i);
                currLevel.get(i).right = newCurr.get(i + 1);
            }
            currLevel = newCurr;
        }
        br.close();
    ...
    public static void main(String[] args) throws IOException {
        ...
        // [not for credit]
        // you can implement NumberTriangle's maxPathSum method if you want to try to
        // solve
